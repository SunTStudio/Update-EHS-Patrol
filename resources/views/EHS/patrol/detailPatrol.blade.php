...
@section('content')
    ...
    @php
        ...
        $deptHeadPIC = App\Models\user_has_area::where('area_id', $laporan->area_id)
                        ...
                        @if (auth()->user()->hasRole('EHS'))
                            <div class="ibox-tools">
                                ...
                                <ul class="dropdown-menu dropdown-user">
                                    <li><a href="{{ route('patrolEhsEdit', ['id' => $laporan->id]) }}"
                                            class="dropdown-item">Edit Data</a>
                                    </li>
                                </ul>
                            </div>
                        @endif
                    ...
                    <div class="ibox-content">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4"><input type="text" disabled=""
                                    placeholder="{{ $laporan->area_patrol->name }}" class="form-control"></div>
            ...
            <div class="col-lg-12">
                                ...
                                @if ($laporan->deleted_at == null)
                                    @role('EHS')
                                        <a class="btn btn-block btn-primary compose-mail"
                                            href="{{ route('createForm', ['id' => $laporan->id]) }}">Buat Temuan</a>
                                    @endrole
                                @endif
                            </div>
                            <div class="col-sm-2 mx-3 m-sm-0 text-end">
                                @if ($exportVisible)
                                    <form method="post" action="{{ route('exportLaporan') }}">
                                        ...
                                        <button class="btn btn-block btn-info compose-mail" type="submit">Export
                                            laporan</button>
                                    </form>
                                @endif
                            </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if ($temuan->ACC_Dept_Head_PIC_At == null && $temuan->progress <= 10.0)
                                    ...
                                    @foreach ($temuans as $temuan)
                                        <tr>
                                            ...
                                            <td class="text-center" style="font-size:12px">
                                                {{ date('d/m/Y', strtotime($temuan->created_at)) }}</td>
                                            <td class="text-center" style="font-size:12px">
                                                {{ date('d/m/Y', strtotime($temuan->deadline_date)) }}</td>
                                            ...
                                            @elseif ($temuan->ACC_Dept_Head_PIC_At == null && $temuan->progress < 10.0)
                                                @if ($temuan->ACC_Dept_Head_PIC_At == null && $temuan->progress == 0)
                                                    <td class="text-center" style="font-size:12px"><span @if (date('d/m/Y', strtotime($temuan->deadline_date)) < date('d/m/Y')) class="pops" @else class="pou" @endif
                                                            >{{ $temuan->progress }}/10</span><br><span>OPEN</span>
                                                    </td>
                                                @else
                                                    <td class="text-center" style="font-size:12px"><span
                                                            @if (date('d/m/Y', strtotime($temuan->deadline_date)) < date('d/m/Y')) class="pop" @else class="pou" @endif
                                                            >{{ $temuan->progress }}/10</span><br><span>{{ $temuan->progress * 10 }}%</span>
                                                    </td>
                                                @endif
                                                <td class="text-center" style="font-size:12px">{{ $temuan->PIC->name }}
                                                </td>
                                            @elseif ($temuan->ACC_Dept_Head_PIC_At == null && $temuan->progress == 10.0)
                                                <td class="text-center" style="font-size:12px"><span
                                                        class="pou">{{ $temuan->progress }}/10</span><br><span>Waiting
                                                        Approval</span></td>
                                                <td class="text-center" style="font-size:12px">
                                                    {{ $deptHeadPIC->user->name }}</td>
                                            @elseif ($temuan->verify_submit_at == null)
                                                <td class="text-center" style="font-size:12px"><span
                                                        class="pie">{{ $temuan->progress }}/10</span><br><span>Waiting
                                                        Verify</span></td>
                                                <td class="text-center" style="font-size:12px">
                                                    {{ $temuan->auditor->name }}</td>
                                            @elseif ($temuan->ACC_Dept_Head_EHS_At == null && $temuan->progress == 12)
                                                <td class="text-center" style="font-size:12px"><span
                                                        class="fa fa-check text-warning"></span><br><span>Waiting
                                                        Approval</span></td>
                                                <td class="text-center" style="font-size:12px">{{ $deptHeadEHS->name }}
                                                </td>
                                            @else
                                                <td class="text-center" style="font-size:12px"><span
                                                        class="fa fa-check text-navy"></span><br><span>CLOSED</span></td>
                                                <td class="text-center" style="font-size:12px"> - </td>
                                            @endif
                                            <td class="text-center" style="font-size:12px"><a
                                                    class="btn btn-info btn-block compose-mail"
                                                    href="{{ route('detailLaporan', ['id' => $temuan->id]) }}">See
                                                    Detail</a> </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                    ...
                                    <div class="row mt-4 mt-sm-0" style="margin-left:0">
                                        ...
                                        <div class="col-sm-6 mx-3 m-sm-0 text-end">
                                            <form id="submitForm" method="post"
                                                action="{{ route('needApproveTemuanPICALL') }}">
                                                @csrf
                                                <input type="hidden" value="{{ $laporan->id }}" name="patrol_id">
                                                <button id="submitButton" class="btn btn-block btn-success compose-mail"
                                ...
                                @if ($verifikasiVisible)
                                    <div class="row mt-4 mt-sm-0" style="margin-left:0">
                                        ...
                                        <div class="col-sm-6 mx-3 m-sm-0 mb-2 text-end inline">
                                            <form id="submitForm" method="post"
                                                action="{{ route('needVerifikasiEhsAll') }}">
                                                @csrf
                                                <input type="hidden" value="{{ $laporan->id }}" name="patrol_id">
                                                <button id="submitButton" class="btn btn-block btn-success compose-mail"
                            ...
                            <div class="row mt-4 mt-sm-0" style="margin-left:0">
                                ...
                                <div class="col-sm-6 mx-3 m-sm-0 text-end">
                                    <form id="submitForm" method="post" action="{{ route('needApproveTemuanEhsAll') }}">
                                        ...
                                        <input type="hidden" value="{{ $laporan->id }}" name="patrol_id">
                                        <button id="submitButton" class="btn btn-block btn-success compose-mail"
                                            type="submit">Kirim email minta approval depthead EHS</button>
                                    </form>
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($DeptEHSApproveVisible)
                            ...
                            <div class="col-sm-6 mx-3 m-sm-0 text-end">
                                <form id="submitForm" method="post" action="{{ route('approveDeptheadEhsAll') }}">
                                    ...
                                    <input type="hidden" value="{{ $laporan->id }}" name="patrol_id">
                                    <button id="submitButton" class="btn btn-block btn-success compose-mail"
                                        type="submit">Kirim email selesai temuan</button>
                                </form>
                            </div>
                    </div>
                    @endif
