...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('tipeTable') && $request->input('tipeTable') == 'semua') {
                ...
                if (
                    ...
                    $query->where('auditor_id', auth()->user()->id)->whereBetween('created_at', [$timeStart . ' 00:00:00', $timeEnd . ' 23:59:59']);
                } elseif (
                    auth()
                        ->user()
                        ->hasRole(['Departement Head PIC', 'PIC'])
                ) {
                    $query
                        ->select('laporan.*')
                        ->join('user_has_areas', 'user_has_areas.area_id', '=', 'laporan.area_id')
                        ->where('user_has_areas.user_id', auth()->user()->id)
                        ->whereBetween('laporan.created_at', [$timeStart . ' 00:00:00', $timeEnd . ' 23:59:59']);
                }
            }
            ...
            if (empty($request->input('search.value'))) {
                $posts = $query->offset($start)->limit($limit)->orderBy($orderColumnName, $dir)->orderBy('progress', 'asc')->orderBy($orderColumnName2, $dir2)->get();
            } else {
                $search = $request->input('search.value');

                $posts = $query
                    ->where('temuan', 'LIKE', "%{$search}%")
                    ->offset($start)
                    ->limit($limit)
                ...
                $totalFiltered = $query->where('temuan', 'LIKE', "%{$search}%")->count();
            }
            ...
            if (!empty($posts)) {
                foreach ($posts as $index => $post) {
                    ...
                    if ($post->ACC_Dept_Head_EHS_At == null && $post->deleted_at !== null) {
                    ...
                    } elseif ($post->ACC_Dept_Head_PIC_At == null && $post->progress < 10.0) {
                        ...
                        $nestedData['assign_to'] =
                            ...
                            user_has_area::where('area_id', $post->area_id)
                                ->whereHas('user.roles', function ($query) {
                                    $query->where('name', 'Departement Head PIC');
                                })
                                ->first()->user->name .
                            '</td>';
                    } elseif ($post->verify_submit_at == null) {
                    ...
                    } elseif ($post->ACC_Dept_Head_EHS_At == null && $post->progress > 10) {
        ...
        return view('EHS/table', ['title' => 'Laporan Table', 'active' => 'Laporan Saya', 'halaman' => 'Laporan', 'areas' => $areas]);
    }
    ...
    public function index_all()
    {
        ...
        return view('EHS/alltable', ['title' => 'Laporan Table', 'active' => 'Semua Laporan', 'halaman' => 'Laporan', 'areas' => $areas]);
    }
    ...
    public function create($id)
    {
        ...
        $PIC = user_has_area::where('area_id', $laporan->area_id)
            ->where('role_id', $rolePIC->id)
            ->first();

        return view('EHS/createform', ['title' => 'Pembuatan Laporan', 'active' => 'Pembuatan Laporan', 'halaman' => 'Laporan', 'laporan' => $laporan, 'PIC' => $PIC]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $roleDeptHeadEHS = Role::findByName('Departement Head EHS');
        ...
        if ($validateData['rank'] == 'A') {
            $EHSandDepts = DB::table('model_has_roles as role')
                ...
                ->where(function ($query) {
                    $roleDeptHeadEHS = Role::findByName('Departement Head EHS');
                    $roleEHS = Role::findByName('EHS');
                    $query->where('role.role_id', $roleEHS->id)->orWhere('role.role_id', $roleDeptHeadEHS->id);
                })
                ->where('role.model_id', '!=', auth()->user()->id)
                ->distinct()
                ->get();
            foreach ($EHSandDepts as $EHSandDept) {
                if ($EHSandDept->role_id == $roleDeptHeadEHS->id) {
        ...
        $Dept_Head_EHS = DB::table('model_has_roles')
            ->where('role_id', $roleDeptHeadEHS->id)
            ->first();
    ...
    public function detail($id)
    {
        ...
        foreach ($laporan->area->area as $pic) {
            if ($pic->user->roles()->where('name', 'PIC')->exists()) {
        ...
        return view('EHS/detailform', ['title' => 'Detail Laporan', 'halaman' => 'Laporan', 'active' => 'Laporan Saya', 'laporan' => $laporan, 'PIC_area' => $pic_area, 'Dept_Head_PIC_area' => $Dept_Head_pic_area]);
    }
    ...
    public function edit($id)
    {
        ...
        if (
            !auth()
                ->user()
                ->hasRole(['Departement Head EHS', 'EHS'])
        ) {
        ...
        return view('EHS/editform', ['title' => 'Edit Laporan', 'halaman' => 'Laporan', 'active' => 'Laporan Saya', 'laporan' => $laporan, 'areas' => $area, 'PICs' => $PIC]);
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if ($genba) {
            if ($laporan->auditor_id != auth()->user()->id && !auth()->user()->hasRole('Departement Head EHS') && !auth()->user()->hasRole('EHS')) {
            ...
            if (
                !auth()
                    ->user()
                    ->hasAnyRole(['Departement Head EHS', 'EHS'])
            ) {
    ...
    public function verify_laporan(Request $request)
    {
        ...
        $dataLama->update(['alasan_penolakan' => null, 'progress' => 12, 'verify_submit_at' => now()]);
    ...
    public function create_penolakan($id)
    {
        ...
        foreach ($dataLama->area->area as $pic) {
            if (
                auth()
                    ->user()
                    ->hasRole(['Departement Head PIC']) &&
                auth()->user()->id == $pic->user_id
            ) {
                $is_pic = true;
            }
        }
        ...
        if (
            !auth()
                ->user()
                ->hasRole(['Departement Head PIC', 'Departement Head EHS', 'EHS'])
        ) {
        ...
        return view('EHS/formPenolakan', ['title' => 'Pembuatan Laporan Perbaikan', 'halaman' => $halaman, 'active' => $active, 'laporan' => $laporan, 'areas' => $area, 'PICs' => $PIC]);
    }
    ...
    public function requestDueDateLanjutan(Request $request, $id)
    {
        ...
        $deskripsi = auth()->user()->name . ' melakukan Request Due Date Lanjutan Berupa, ' . $validateData['alasan_due_date_lanjutan'];
        $data = new activity_log();
        $data->user_id = auth()->user()->id;
        $data->auditor_id = $laporan->auditor_id;
        $data->laporan_id = $laporan->id;
    ...
    public function konfirmasiDueDateLanjutan(Request $request, $id, $status)
    {
        ...
        if ($status == 'acc') {
            ...
            $data->status_read_ehs = true;
            $data->deskripsi = auth()->user()->name . ' telah melakukan approval terhadap Due Date laporan';
            ...
            $data->status_read_ehs = true;
            $data->deskripsi = auth()->user()->name . ' telah melakukan penolakan terhadap Due Date laporan';
    ...
    public function update_tolak_laporan(Request $request)
    {
        ...
        foreach ($area->area as $pic) {
            if (
                ...
                } elseif ($pic->user->hasRole(['Departement Head PIC']) && auth()->user()->id != $pic->user_id) {
                    $count_notification = user_notification_count::where('user_id', $pic->user_id)->first();
    ...
    public function create_laporan_penanggulangan($id)
    {
        ...
        if (auth()->user()->hasRole('EHS')) {
            $userRole = 'EHS';
        } elseif (auth()->user()->hasRole('Departement Head PIC')) {
            $userRole = 'Departement Head PIC';
        } elseif (auth()->user()->hasRole('Departement Head EHS')) {
            $userRole = 'Departement Head EHS';
        ...
        return view('PIC/penanggulangan', ['title' => 'Pembuatan Laporan Perbaikan', 'halaman' => 'Laporan', 'active' => 'Laporan Saya', 'laporan' => $laporan, 'areas' => $area, 'PICs' => $PIC, 'inputSementara' => $inputSementara, 'deadline_date_lanjutan' => $laporan->deadline_date_lanjutan]);
    }
        ...
        if (!empty($kolomBerubah)) {
            ...
            foreach ($kolomBerubah as $kolom => $nilai) {
                if ($kolom !== 'deadline_date' && $kolom !== 'updated_at' && $kolom !== 'PIC_submit_at') {
                    ...
                    } elseif ($kolom == 'progress') {
                        $perubahan[] = "$kolom sebanyak " . $nilai * 10 . '%';
                    } elseif ($kolom == 'temporary_solution') {
                    } else {
                        $perubahan[] = "$nilai";
                    }
                }
    ...
    public function Accept_PIC(Request $request)
    {
        ...
        $laporan->update(['ACC_Dept_Head_PIC_At' => now(), 'dept_pic_id' => auth()->user()->id, 'progress' => 11]);
        $laporan->save();
        ...
        // melihat id dept head
        $roleDeptEHS = Role::findByName('Departement Head EHS');

        $Dept_Head_EHS = DB::table('model_has_roles')
            ->where('role_id', $roleDeptEHS->id)
            ->first();
